...
var _addLectureValidation = __webpack_require__(125);
...
var API = function () {
    ...
    _createClass(API, [{
        ...
        value: function initialize(Obj) {
            ...
            this.schools = {};
            this.error = ['err'];
            var lecturesArray = Object.keys(Obj);
            lecturesArray.map(function (id) {
                var teacherId = [Obj[id].teacher.id];

                _this.teachers[teacherId] = {
                    id: Obj[id].teacher.id,
                    name: Obj[id].teacher.name,
                    lectures: [].concat(_toConsumableArray(_this.teachers[teacherId] && _this.teachers[teacherId].lectures || []), [id])
                };

                var classRoomId = [Obj[id].classRoom.id];
                var classRoom = Obj[id].classRoom;

                _this.classRooms[classRoomId] = {
                    id: Obj[id].classRoom.id,
                    name: classRoom.name,
                    maxStudents: classRoom.maxStudents,
                    lectures: [].concat(_toConsumableArray(_this.classRooms[classRoomId] && _this.classRooms[classRoomId].lectures || []), [id])
                };

                var schoolId = [Obj[id].school.id];

                _this.schools[schoolId] = {
                    id: Obj[id].school.id,
                    name: Obj[id].school.name,
                    lectures: [].concat(_toConsumableArray(_this.schools[schoolId] && _this.schools[schoolId].lectures || []), [id])
                };

                LECTURES = _this.lectures;
                TEACHERS = _this.teachers;
                CLASSROOMS = _this.classRooms;
                SCHOOLS = _this.schools;
        ...
        key: 'filter',
        value: function filter(FIREBASEDATA, dF, dT, t, sC, cR) {
            var displayedItem = FIREBASEDATA;
        ...
        key: 'setLecture',
        value: function setLecture() {
            var lect = document.querySelector('#lecture').value;
            var teacher = document.querySelector('#teacher').value;
            var comp = document.querySelector('#company').value;
            var sch = document.querySelector('#school').value;
            var inputStartTime = document.querySelector('#startTime').value;
            var inputEndTime = document.querySelector('#endTime').value;
            var classRoom = document.querySelector('#classRoom').value;
            var amountOfStudents = document.querySelector('#amountOfStudents').value;
            var loc = document.querySelector('#location').value;
            var inputDate = document.querySelector('#date').value;
            var errArr = [];
            var startT = inputStartTime.split(':');
            var endT = inputEndTime.split(':');
            var date = inputDate.split('-');
            var secFrom = new Date(date[0], date[1] - 1, date[2], startT[0], startT[1]).getTime();
            var secTo = new Date(date[0], date[1] - 1, date[2], endT[0], endT[1]).getTime();

            var validatedData = this.fullValidation(lect, teacher, comp, sch, inputStartTime, inputEndTime, classRoom, amountOfStudents, loc, inputDate, errArr, startT, endT, date, secFrom, secTo);

            if (validatedData) {
                var newPostKey = firebase.database().ref().child('posts').push().key; // генерим уникальный id
        ...
        key: 'fullValidation',
        value: function fullValidation(lect, teacher, comp, sch, inputStartTime, inputEndTime, classRoom, amountOfStudents, loc, inputDate, errArr, startT, endT, date, secFrom, secTo) {
            if (!(0, _addLectureValidation.checkInputFields)(inputDate, lect, teacher, comp, sch, inputStartTime, inputEndTime, classRoom, amountOfStudents, loc)) {
                errArr.push('Все поля должны быть заполнены');

                return false;
            }

            (0, _addLectureValidation.checkInstances)(inputDate, lect, teacher, comp, sch, inputStartTime, inputEndTime, classRoom, amountOfStudents, loc);

            if (!(0, _addLectureValidation.checkIfStartTimeIsEarlierThanEndTime)(secFrom, secTo, errArr)) {
                errArr.push('Начало лекции не может быть позже ее конца');

                return false;
            }
            // проверка учителя
            var teacherId = (0, _addLectureValidation.matchByName)(TEACHERS, teacher); // если id учителя есть в базе - этот id присваивается переменной
            if (teacherId) {
                // если id учителя найден - мы ищем лекции, которые ведет этот учитель и сверяем по времени, есть ли пересечения
                var noIntersections = (0, _addLectureValidation.matchLectures)(TEACHERS, LECTURES, teacherId, secFrom, secTo);
                if (!noIntersections) {
            ...
            if (!teacherId) teacherId = (0, _addLectureValidation.generateNewId)(); // если его нет - генерится новый id
            // проверка школы
            var schoolId = (0, _addLectureValidation.matchByName)(SCHOOLS, sch); // если id школы есть в базе - этот id присваивается переменной
            if (schoolId) {
                // если id школы найден - мы ищем лекции в базе, которые идут у этой школы и сверяем по времени, есть ли пересечения
            ...
            if (!schoolId) schoolId = (0, _addLectureValidation.generateNewId)(); // если его нет - генерится новый
            // проверка аудитории
            var classRoomId = (0, _addLectureValidation.matchByName)(CLASSROOMS, classRoom); // если id аудитории есть в базе - этот id присваивается переменной
            if (classRoomId) {
                // если id аудитории найден - мы ищем лекции в базе, которые идут в этой аудитории и сверяем по времени, есть ли пересечения
                var _noIntersections2 = (0, _addLectureValidation.matchLectures)(CLASSROOMS, LECTURES, classRoomId, secFrom, secTo);
                if (!_noIntersections2) {
                    // если есть пересечения - выводим ошибку
            ...
            var classRoomCapacity = (0, _addLectureValidation.addClassRoomCapacity)(CLASSROOMS, classRoomId);
            ...
            if (!classRoomId) classRoomId = (0, _addLectureValidation.generateNewId)(); // если его нет - генерится новый id
...
var _match2 = __webpack_require__(163);
...
var _matchRoutes = __webpack_require__(164);
...
function getProperDate(seconds) {
    var month = new Date(seconds).getMonth() + 1;
...
var database = firebase.database();
var rootRef = database.ref('lectures');
var FIREBASEDATA = [];

var API_GUI = function (_React$Component) {
    _inherits(API_GUI, _React$Component);

    function API_GUI(props) {
        _classCallCheck(this, API_GUI);

        var _this = _possibleConstructorReturn(this, (API_GUI.__proto__ || Object.getPrototypeOf(API_GUI)).call(this, props));

        _this.state = {
            errorList: [''],
            lectures: []
        };
        _this.showSubmissionResult = _this.showSubmissionResult.bind(_this);
        return _this;
    }

    _createClass(API_GUI, [{
        key: 'showSubmissionResult',
        value: function showSubmissionResult() {
            var errArr = _API2.default.setLecture();
            this.setState({
                errorList: errArr
            });
        }
        ...
        value: function render() {
            return _react2.default.createElement(
                ...
                _react2.default.createElement(
                    ...
                    _react2.default.createElement(
                        'h1',
                        { className: 'api__header' },
                        'API Graphic User Interface'
                    ),
                    ...
                    _react2.default.createElement('br', null),
                    _react2.default.createElement('input', { id: 'amountOfStudents', type: 'defaultValue', defaultValue: '20', placeholder: '\u041A\u043E\u043B-\u0432\u043E   \u0441\u0442\u0443\u0434\u0435\u043D\u0442\u043E\u0432' }),
                    _react2.default.createElement('br', null),
                    _react2.default.createElement('input', { id: 'location', type: 'text', defaultValue: '\u041C\u043E\u0441\u043A\u0432\u0430', placeholder: '\u0413\u043E\u0440\u043E\u0434' }),
                    _react2.default.createElement('br', null),
                    _react2.default.createElement(
                        'button',
                        { id: 'submit', onClick: this.showSubmissionResult },
                        'Submit'
                    ),
                    this.state.errorList.map(function (error, index) {
                        return _react2.default.createElement(Show, {
                            key: index,
                            error: error
                        });
                    })
...
function DisplayLectures(props) {
    ...
    return _react2.default.createElement(
        ...
        _react2.default.createElement('input', { id: 'lecture', defaultValue: lecture }),
        _react2.default.createElement('input', { defaultValue: teacher }),
        _react2.default.createElement('input', { defaultValue: company }),
        _react2.default.createElement('input', { defaultValue: school }),
        _react2.default.createElement('input', { defaultValue: startTime }),
        _react2.default.createElement('input', { defaultValue: endTime }),
        _react2.default.createElement('input', { defaultValue: classRoom }),
        _react2.default.createElement('input', { defaultValue: classRoomCapacity }),
        _react2.default.createElement('input', { defaultValue: location }),
        _react2.default.createElement('input', { defaultValue: isDeleted }),
        _react2.default.createElement(
            'button',
            { id: 'changeBtn' },
            'Change'
...
DisplayLectures.propTypes = {
    ...
    startTime: _react2.default.PropTypes.string.isRequired,
    endTime: _react2.default.PropTypes.string.isRequired,
    ...
    isDeleted: _react2.default.PropTypes.string.isRequired
...
var _convert = __webpack_require__(126);
...
var _schedule = __webpack_require__(123);
...
function _inherits(subClass, superClass) { if (typeof superClass !== "function" && superClass !== null) { throw new TypeError("Super expression must either be null or a function, not " + typeof superClass); } subClass.prototype = Object.create(superClass && superClass.prototype, { constructor: { value: subClass, enumerable: false, writable: true, configurable: true } }); if (superClass) Object.setPrototypeOf ? Object.setPrototypeOf(subClass, superClass) : subClass.__proto__ = superClass; } // конвертирую падеж названия месяца (прим. январь -> января)
// исправляю нюанс вывода времени (прим. 12:0 -> 12:00 (с учетом проверки))
...
var ScheduleApp = function (_React$Component) {
    _inherits(ScheduleApp, _React$Component);

    function ScheduleApp() {
        _classCallCheck(this, ScheduleApp);

        var _this = _possibleConstructorReturn(this, (ScheduleApp.__proto__ || Object.getPrototypeOf(ScheduleApp)).call(this));
    ...
    _createClass(ScheduleApp, [{
        ...
        value: function componentDidMount() {
            ...
            rootRef.on('value', function (snap) {
                ...
                for (var x in Obj) {
                    if (Object.prototype.hasOwnProperty.call(Obj, x)) {
                        if (Obj[x].isDeleted === false) {
                            // если флаг isDeleter === true, мы не показываем эту лекцию в списке
                            FIREBASEDATA.push(Obj[x]);
                        }
                    }
                }
                FIREBASEDATA = FIREBASEDATA.sort(function (a, b) {
                    // сортируем лекции дате
                ...
                classRooms = _API2.default.classRooms;
                teachers = _API2.default.teachers;
                schools = _API2.default.schools;

                for (var i = 0; i < FIREBASEDATA.length; i++) {
                    var path = FIREBASEDATA[i].date;
                    dates.push((0, _properDate2.default)(path));
                }
                ...
                dates = [].concat(_toConsumableArray(new Set(dates)));
        ...
        value: function filter() {
            ...
            var sC = document.querySelector('#school').value;
            var cR = document.querySelector('#classRoom').value;
            var displayedItem = _API2.default.filter(FIREBASEDATA, dF, dT, t, sC, cR);
        ...
        value: function render() {
            return _react2.default.createElement(
                ...
                _react2.default.createElement(
                    ...
                    _react2.default.createElement(
                        'div',
                        { className: 'schedule-container__filters' },
                        _react2.default.createElement(
                            'select',
                            { id: 'dateFrom', className: 'input', onChange: this.filter },
                            _react2.default.createElement(
                                'option',
                                null,
                                '\u0421'
                            ),
                            dates.map(function (key, index) {
                        ...
                        _react2.default.createElement(
                            'select',
                            { id: 'dateTo', className: 'input', onChange: this.filter },
                            _react2.default.createElement(
                                'option',
                                null,
                                '\u041F\u043E'
                        ...
                        _react2.default.createElement(
                            'select',
                            { id: 'teacher', className: 'input', onChange: this.filter },
                            _react2.default.createElement(
                                'option',
                                null,
                                '\u0412\u0441\u0435 \u043F\u0440\u0435\u043F\u043E\u0434\u0430\u0432\u0430\u0442\u0435\u043B\u0438'
                        ...
                        _react2.default.createElement(
                            'select',
                            { id: 'school', className: 'input', onChange: this.filter },
                            _react2.default.createElement(
                                'option',
                                { className: 'option' },
                                '\u0412\u0441\u0435 \u0448\u043A\u043E\u043B\u044B'
                        ...
                        _react2.default.createElement(
                            'select',
                            { id: 'classRoom', className: 'input', onChange: this.filter },
                            _react2.default.createElement(
                                'option',
                                null,
                                '\u0412\u0441\u0435 \u0430\u0443\u0434\u0438\u0442\u043E\u0440\u0438\u0438'
                    ...
                    this.state.displayedItem.map(function (el, index) {
                        return _react2.default.createElement(_schedule2.default, {
                            ...
                            company: el.company,
                            room: el.classRoom,
                            startTime: (0, _time2.default)(el.date),
                            classRoom: el.classRoom.name,
                            endTime: (0, _time2.default)(el.endTime)
                        });
                    })
                )
            );
...
function Schedule(props) {
  ...
  return (// далее отрисуем див
    _react2.default.createElement(
      "div",
      { className: "schedule-container__line" },
      _react2.default.createElement(
        "div",
        { className: "schedule-container__line__block_1" },
        _react2.default.createElement(
          "div",
          { className: "schedule-container__line__block_1__date" },
          _react2.default.createElement(
            "span",
            { className: "schedule-container__line__block_1__date__number" },
            date
          ),
          _react2.default.createElement(
            "span",
            { className: "schedule-container__line__block_1__month" },
            month
          )
        )
      ),
      _react2.default.createElement(
        "div",
        { className: "schedule-container__line__block_2" },
        _react2.default.createElement(
          "div",
          { className: "schedule-container__line__block_2__lecture" },
          lecture
        ),
        _react2.default.createElement(
          "div",
          { className: "schedule-container__line__block_2__lecturer" },
          _react2.default.createElement(
            "a",
            { href: "#" },
            teacher
        ...
        _react2.default.createElement(
          "div",
          { className: "schedule-container__line__block_2__company" },
          company
        )
      ),
      _react2.default.createElement(
        "div",
        { className: "schedule-container__line__block_3" },
        _react2.default.createElement(
          "div",
          { className: "schedule-container__line__block_3__school" },
          school
        )
      ),
      _react2.default.createElement(
        "div",
        { className: "schedule-container__line__block_4" },
        _react2.default.createElement(
          "div",
          { className: "schedule-container__line__block_4__time" },
        ...
        _react2.default.createElement(
          "div",
          { className: "schedule-container__line__block_4__room" },
          _react2.default.createElement("i", { className: "fa fa-home", "aria-hidden": "true" }),
          classRoom
        ),
        _react2.default.createElement(
          "div",
          { className: "schedule-container__line__block_4__location" },
...
var _api_gui = __webpack_require__(119);

var _api_gui2 = _interopRequireDefault(_api_gui);
...
_reactDom2.default.render(_react2.default.createElement(
  ...
  _react2.default.createElement(_reactRouter.Route, { path: '/api', component: _api_gui2.default })
), app);
...
exports.checkIfStartTimeIsEarlierThanEndTime = checkIfStartTimeIsEarlierThanEndTime;
exports.matchByName = matchByName;
exports.matchLectures = matchLectures;
exports.generateNewId = generateNewId;
exports.addClassRoomCapacity = addClassRoomCapacity;
function checkInputFields(inputDate, lect, teacher, comp, sch, inputStartTime, inputEndTime, classRoom, amountOfStudents, loc) {
    if (inputDate === '' || lect === '' || teacher === '' || comp === '' || sch === '' || inputStartTime === '' || inputEndTime === '' || classRoom === '' || amountOfStudents === '' || loc === '') {
        return false;
    }
...
function checkInstances(inputDate, lect, teacher, comp, sch, inputStartTime, inputEndTime, classRoom, amountOfStudents, loc) {
...
function checkIfStartTimeIsEarlierThanEndTime(secFrom, secTo) {
    if (secFrom > secTo) {
        return false;
    }
...
function checkIntersection(newStart, newEnd, existStart, existEnd) {
    if (newStart <= existStart && newEnd <= existStart || newStart >= existEnd && newEnd >= existEnd) {
        // проверяем чтобы не было пересечений с другими лекциями
        return true;
    }
...
function matchByName(entity, inputVal) {
    ...
    if (foundObj) {
        // если ID в базе школ найден, тогда присваиваем его переменной
        return foundObj.id;
    }
...
function matchLectures(entity, database, id, secFrom, secTo) {
...
function addClassRoomCapacity(CLASSROOMS, classRoomId) {
    if (classRoomId) {
        // если ID аудитории найден в базе, то этой аудитории присваивается своя вместимось
        return CLASSROOMS[classRoomId].maxStudents;
    }

    if (!classRoomId) {
        // если ID аудитории не найден в базе, то у пользователя запрашивается вместимость аудитории
        while (!classRoomId) {
...
var _makeStateWithLocation = __webpack_require__(83);
...
var ReactDOMDebugTool = __webpack_require__(188);
...
if (process.env.NODE_ENV !== 'production') {
  ...
  var ReactNativeOperationHistoryDevtool = __webpack_require__(210);
  var ReactComponentTreeDevtool = __webpack_require__(182);
